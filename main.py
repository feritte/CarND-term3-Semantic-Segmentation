...
def _conv_layer(layer, num_classes):
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    conv7 = _conv_layer(vgg_layer7_out, num_classes)

    deconv7 = _deconv_layer(conv7, num_classes, 4, 2)
    conv4 = _conv_layer(vgg_layer4_out, num_classes)
    output = tf.add(deconv7, conv4)

    output = _deconv_layer(output, num_classes, 4, 2)
    conv3 = _conv_layer(vgg_layer3_out, num_classes)
    output = tf.add(output, conv3)

    nn_last_layer = _deconv_layer(output, num_classes, 16, 8)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        save_path = saver.save(sess, "./fcn.ckpt")
